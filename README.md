...
4. **normalizes** each remaining declaration for comparison (skipping the contents of quoted strings and `url()` throughout): whitespace, value case, zero-value length/percentage units (`0px`/`0svh`/`0cqw` → `0`—angle/time/frequency/resolution units like `0deg`/`0s` are left alone, since unitless zero isn’t valid there), redundant decimal zeros (`.5`/`0.5`/`0.50` → `.5`, `1.0` → `1`), and the `border`/`outline` `none` ↔ `0` equivalence
